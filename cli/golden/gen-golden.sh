...
check "ts: NoRoute fallback path"      "$(grep -c 'ThingService/NoRoute' "$OUT/ts/easyrpc/$K/kitchen_easyrpc.ts" 2>/dev/null)"
...
check "go: REST get path"              "$(grep -c '/v1/things/{id}' "$OUT/go/easyrpc/$K/kitchen.easyrpc.go" 2>/dev/null)"
