#!/bin/bash
# Start/stop the TypeScript conformance server (pidfile tracked).
# start [port]  (SERVER_PROTO=h1|h2c; default h1 — Node cannot mux h1+h2c on one socket)
set -u
...
start(){
  ...
  HTTP_PROTOCOL="${SERVER_PROTO:-h1}" PORT="$PORT" setsid node /home/user/easy-utils/easy-rpc-ts/dist/srv.js >"$LOG" 2>&1 </dev/null &
  echo $! > "$PIDFILE"
